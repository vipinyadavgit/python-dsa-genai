...
# After training, Random Forest gives every feature a score (0 to 1)
...
# Step 1: get the list of all column names (features we used as input)
feature_names = X.columns.tolist()

# Step 2: get the importance score for each feature from the trained Random Forest
importance_scores = rf.feature_importances_

# Step 3: put feature names and their scores side by side in a table
feat_imp_df = pd.DataFrame({
    'Feature'   : feature_names,
    'Importance': importance_scores
})

# Step 4: sort the table so most important features appear at the top
feat_imp_df = feat_imp_df.sort_values('Importance', ascending=False)

# Step 5: keep only top 15 so the chart is not too crowded
feat_imp_df = feat_imp_df.head(15).reset_index(drop=True)

print("\nTop 15 Features that influence Attrition the most:")
print(feat_imp_df.round(4))

# Step 6: draw a horizontal bar chart
# Longest bar = most important feature
plt.figure(figsize=(10, 6))
plt.barh(feat_imp_df['Feature'][::-1], feat_imp_df['Importance'][::-1],
         color='steelblue', edgecolor='black')
plt.xlabel("Importance Score")
plt.title("Top 15 Features Driving Employee Attrition (Random Forest)")
plt.tight_layout()
plt.savefig("output_plots/08_feature_importance.png", dpi=150, bbox_inches='tight')
plt.show()
